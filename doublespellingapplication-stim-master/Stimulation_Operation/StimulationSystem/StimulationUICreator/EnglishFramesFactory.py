...
class EnglishFramesFactory(BasicSchemaStimulationFramesFactory):
...
if __name__ == '__main__':
    ...
    target_table = {'DISPLAYCHAR': display_char,
                    'FREQUENCY': frequency,
                    'PHASE': phase}
    t = ShengmuFramesFactory()
    t.initial(StimulationUIParameters)
    frames = t.get_frames(target_table)
    images = []
    for frame in range(0, 600):
        plt.imsave('D:/postgraduate/doublespelling/Stimulation_Operation/StimulationSystem/stimulationInformationFolder/DoubleSpelling/0/'+str(frame)+'.png', frames.frame_set[frame])
        images.append(plt.imread('D:/postgraduate/doublespelling/Stimulation_Operation/StimulationSystem/stimulationInformationFolder/DoubleSpelling/0/'+str(frame)+'.png'))
        #print(type(images[0]))
    ...
    for j in range(0, 600):
        image_sti.append(visual.ImageStim(mywin, image='D:/postgraduate/doublespelling/Stimulation_Operation/StimulationSystem/stimulationInformationFolder/DoubleSpelling/0/'+str(j)+'.png', pos=[0, 0], size=[1920, 1080], units='pix', flipVert=False))
