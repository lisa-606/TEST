...
from MessageQueue.exception.Exceptions import NoConfigFileException, WrongMessageValueType
from MessageQueue.interface.CommunicationProducerInterface import CommunicationProducerInterface
...
class CommunicationProducer(CommunicationProducerInterface):
    ...
    def send(self, topic, value, key=None):
        ...
        if not type(value) == type(b"a"):
            ...
            remainMsg = self.producer.flush(timeout=1)
            if remainMsg > 0:
                return False
            else:
                return True
