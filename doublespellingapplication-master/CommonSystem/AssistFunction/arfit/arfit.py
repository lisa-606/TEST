import numpy as np
from doubleSpellingApplication.CommonSystem.AssistFunction.arfit.arord import arord
from doubleSpellingApplication.CommonSystem.AssistFunction.arfit.arqr import arqr
...
def arfit(v, pmin, pmax, selector, no_const):
    ...
    if type(pmin) is not int or type(pmax) is not int:
        raise Exception("error: Order must be integer.")
        # print("error: Order must be integer.")

    if pmax < pmin:
        raise Exception("error: PMAX must be greater than or equal to PMIN.")
    ...
    if selector is None:
        ...
        if no_const is 'zero':
            ...
            raise Exception("error: Bad argument. Usage:  [w,A,C,SBC,FPE,th]=AR(v,pmin,pmax,SELECTOR,''zero'')")
    ...
    if ne <= npmax:
        raise Exception("Time series too short.")
