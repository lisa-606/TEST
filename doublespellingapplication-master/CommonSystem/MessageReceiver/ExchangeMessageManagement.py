from doubleSpellingApplication.CommonSystem.MessageReceiver.EventManager import EventManager, Event
import queue
from threading import Thread
import uuid
import os
import logging
from doubleSpellingApplication.CommonSystem.MessageReceiver.receive_exchange_message_fcn import receive_exchange_message_fcn
from doubleSpellingApplication.CommonSystem.MessageReceiver.receive_exchange_message_fcn_local_test import receive_exchange_message_fcn_local_test
from doubleSpellingApplication.CommonSystem.MessageReceiver.operate_exchange_message_fcn import operate_exchange_message_fcn
...
initial_config_path = os.path.join(os.path.dirname(__file__), 'config', 'Initial-config.json')
...
class ExchangeMessageManagement:
    def __init__(self, exchange_message_operator, topic_receive, topic_send):
        ...
        topicCreateResult = CommunicationInitial.topicCreate(self.topic_send, initial_config_path)
        ...
        self.consumer.subscribe(self.topic_receive)
    ...
    def initial(self, exchange_message_operator):
        ...
        self.receive_exchange_message_thread = Thread(name = 'receive_exchange_message_thread', target=receive_exchange_message_fcn, args=(self.message_queue, self.consumer, lambda: self.stop_flag,))
        # self.receive_exchange_message_thread = Thread(target=receive_exchange_message_fcn_local_test, args=(self.message_queue,))
        self.operator_exchange_message_thread = Thread(name = 'operator_exchange_message_thread', target=operate_exchange_message_fcn, args=(self.message_queue, self.event_manager, lambda: self.stop_flag,))
    ...
    def delete(self):
        self.logger.debug("ExchangeMessageManagement.delete is called.")
        self.remove_listener_from_operator()
