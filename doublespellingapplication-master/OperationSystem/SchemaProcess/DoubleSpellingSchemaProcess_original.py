...
class DoubleSpellingSchemaProcess(BasicSchemaProcess):
    def __init__(self):
        ...
        self.del_up_text_flag = None

        self.del_up_text_counter = None
    ...
    def initial(self, experiment_information, exchange_message_management):
        ...
        self.candidate_hanzi_display = self.candidate_display_reorder(['上', '', '', '', '', '', '', '', '','下'])
        ...
        self.del_up_text_flag = 0

        self.del_up_text_counter = 0
    ...
    def report(self, result_index):
        ...
        if result_obj.is_punctuation:
            ...
            if len(self.down_list) != 0:
                ...
                if self.down_list[-1].is_shengmuyunmu:

                    j = len(self.down_list)-1
                    for i in range(len(self.down_list)-1, -1, -1):
                        if self.down_list[i].is_shengmuyunmu:
                            j -= 1
                            continue
                        break
                ...
                self.del_up_text_flag = 1

            if self.del_up_text_flag:
                self.del_up_text_counter += 1
                if self.del_up_text_counter == 2:
                    self.up_text = ''
                    self.del_up_text_flag = 0
                    self.del_up_text_counter = 0

        elif result_obj.char == 'Enter':
            self.candidate_hanzi_display = self.candidate_display_reorder_reverse(self.candidate_hanzi_display)
            self.candidate_hanzi_display_id_array = self.candidate_display_reorder_reverse(
                self.candidate_hanzi_display_id_array)
            ...
            if result_obj.is_yunmu and self.mode == 1:
                ...
                if result_obj.char.find('\n') > 0:
                    ...
                    # temp_candidate_hanzi1 = []
                    # temp_candidate_hanzi_id_array1 = []
                    # temp_candidate_hanzi2 = []
                    # temp_candidate_hanzi_id_array2 = []
                    # if self.is_correct_yunmu(sm, yunmu1):
                    #     temp_shengmuyunmu_to_get_candidate_hanzi1 = copy.deepcopy(self.shengmuyunmu_to_get_candidate_hanzi)
                    #     temp_shengmuyunmu_to_get_candidate_hanzi1.append(yunmu1)
                    #     temp_idArray1 = pinyin2ID(temp_shengmuyunmu_to_get_candidate_hanzi1, self.IDTable)
                    #     ch1 = self.converter.getCandidateChar(temp_idArray1)
                    #     temp_candidate_hanzi1 = ch1.character
                    #     temp_candidate_hanzi_id_array1 = ch1.idArray
                    # if self.is_correct_yunmu(sm, yunmu2):
                    #     temp_shengmuyunmu_to_get_candidate_hanzi2 = copy.deepcopy(
                    #         self.shengmuyunmu_to_get_candidate_hanzi)
                    #     temp_shengmuyunmu_to_get_candidate_hanzi2.append(yunmu2)
                    #     temp_idArray2 = pinyin2ID(temp_shengmuyunmu_to_get_candidate_hanzi2, self.IDTable)
                    #     ch2 = self.converter.getCandidateChar(temp_idArray2)
                    #     temp_candidate_hanzi2 = ch2.character
                    #     temp_candidate_hanzi_id_array2 = ch2.idArray
                    #
                    # self.candidate_hanzi = [rv for r in zip(temp_candidate_hanzi1, temp_candidate_hanzi2) for rv in r]
                    # self.candidate_hanzi_id_array = [rv for r in zip(temp_candidate_hanzi_id_array1, temp_candidate_hanzi_id_array2) for rv in r]2
                else:
            ...
            if result_index is 30:
                ...
                selected_index = [0, 2, 4, 6, 8, 16, 18, 28, 30, 36].index(result_index)
                ...
                for i, el in enumerate(selected_char_pinyin_array):
                    if i % 2 == 0:
                        ind = min(index for index, item in enumerate(self.down_list) if item.char == el)
                        if i == 0:
                            self.down_list[ind] = DownListObj(selected_char)
        ...
        message = 'RSLT:' + str(int(result_index))+':'+str(self.mode)+':'+self.up_text+':'+self.down_text+':'+','.join(self.candidate_hanzi_display)
        ...
        '''
        uav_test_list = ["UVDW", "UVUP", "UVTL", "UVTR", "UVFB", "UVFF", "UVFL", "UVFR", "UVLD",
                         "UVTO"]  # , "UVTO", "UVLD"]
